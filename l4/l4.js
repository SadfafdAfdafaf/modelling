...
function get_result() {
  var gen_time_f = get_time_generator;
  var proc_time_f = get_time_processor;
  
  var rcnt_user = Number(r_cnt.value) | 0;
  var qlen = rcnt_user * test_rcnt_factor;
  ...
  var f = function() {
    if (queue.length > max_in_queue)
      max_in_queue = queue.length;
    
    if (!rcnt && !queue.length && !cur_proc_req) {
      res = (max_in_queue > res) ? max_in_queue : res;
      return 0;
    }
  ...
  var simulation_step_func = function() {
    var gen_step = function() {
      proc_time -= Math.min(gen_time, proc_time);
      gen_time = gen_func();
    }
  ...
  for (i = 0; i < test_count; ++i) {
    rcnt = rcnt_user * test_rcnt_factor;
    max_in_queue = 0;
    
    while (f());
  }
  
  result.innerHTML = Math.ceil(res / test_rcnt_factor);
}
